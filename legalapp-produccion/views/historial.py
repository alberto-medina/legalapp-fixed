...
class HistorialScreen(Screen):
    ...
    def _add_card(self, cid, abogado, estado, tipo, tiene_resena=True):
        ...
        info = BoxLayout(orientation="vertical", spacing=dp(4), size_hint_y=None)
        ...
        estado_lbl.bind(width=lambda s, v: setattr(s, "text_size", (v, None)))

        info.add_widget(abogado_lbl)
        info.add_widget(estado_lbl)
        info.bind(minimum_height=info.setter("height"))
        info.bind(height=lambda inst, v: setattr(card, "height", max(dp(112), v + dp(24))))
