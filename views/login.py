...
class LoginScreen(Screen):
    ...
    def _mostrar_login_admin(self):
        ...
        lbl = Label(text='Acceso Admin\nIngrese código secreto:')
        txt = TextInput(password=True, multiline=False, hint_text='Código admin')
        btn = Button(text='Acceder', size_hint_y=None, height=50)
        layout.add_widget(lbl)
        layout.add_widget(txt)
        layout.add_widget(btn)
        popup = Popup(title='Panel Admin', content=layout, size_hint=(0.8, 0.4))
    ...
    def login(self):
        ...
        if ok:
            ...
            if rol == 'cliente' and not email_verified:
                ...
                self.ids.error.text = "Email no verificado. Te redirigimos para activar tu cuenta."
            ...
            # Actualizar FCM token al loguear
            if platform == 'android':
            ...
            self.ids.error.text = error or "Email o contraseña incorrectos"
