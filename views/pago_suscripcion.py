...
def _crear_preferencia_suscripcion(uid, email, monto, especialidades):
    try:
        ...
        body = {
            ...
            "back_urls": {
                "success": "legalapp://suscripcion/success",
                "failure": "legalapp://suscripcion/failure",
                "pending": "legalapp://suscripcion/pending"
            },
...
class PagoSuscripcionScreen(Screen):
    ...
    def on_enter(self):
        self._uid = getattr(session, 'abogado_registrando_uid', None) or getattr(session, 'pending_uid', None)
        self._email = getattr(session, 'pending_email', '')
        self._monto = getattr(session, 'monto_suscripcion', 55000)
        ...
        Clock.schedule_once(lambda dt: self._generar_preferencia(), 0.3)
    ...
    def activar_manual(self):
        # Para pagos en efectivo o por otro medio
        self._activar_suscripcion()
    ...
    def _activar_suscripcion(self):
        ...
        try:
            ...
            fb.enviar_codigo_verificacion(self._email, self._uid)

            session.pending_uid = self._uid
            session.pending_email = self._email

            self.ids.lbl_estado.text = "Suscripcion activada! Verificando email..."
            self.ids.lbl_estado.color = (0.18, 0.80, 0.44, 1)
            self.ids.btn_pagar.disabled = True
            self.ids.btn_verificar.disabled = True
            self.ids.btn_activar_manual.disabled = True
