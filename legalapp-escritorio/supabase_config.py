...
ADMIN_ACTIONS_URL = f"{SUPABASE_URL}/functions/v1/admin-actions"
...
def _invocar_admin_action(action, payload, admin_id_token):
    """Llama a la Edge Function admin-actions, que verifica el id_token de
    Firebase contra la tabla usuarios (rol='admin') antes de ejecutar nada.
    Reemplaza las escrituras directas que antes hacia el cliente con la
    anon key para aprobar abogados, procesar retiros y editar precios."""
    data = payload.copy()
    data["action"] = action
    data["id_token"] = admin_id_token
    return _invocar_edge_function(ADMIN_ACTIONS_URL, data)
...
def listar_abogados(disponibles=True, provincia=None, ciudad=None):
    try:
        ...
        if provincia:
            filters["provincia"] = f"ilike.{provincia}"
        if ciudad and ciudad != "Otras":
            filters["ciudad"] = f"ilike.{ciudad}"
...
def desactivar_suscripcion_abogado(uid, admin_id_token=None):
    """Accion de administrador (bloquear abogado). Requiere admin_id_token,
    ver admin-actions."""
    try:
        data = _invocar_admin_action("desactivar_suscripcion", {"uid": uid}, admin_id_token) or {}
        return bool(data.get("ok"))
    except Exception as e:
        logger.error(f"ERROR desactivar_suscripcion: {e}")
        return False

def reactivar_abogado(uid, admin_id_token=None):
    """Accion de administrador. Requiere admin_id_token, ver admin-actions."""
    try:
        data = _invocar_admin_action("reactivar_abogado", {"uid": uid}, admin_id_token) or {}
        if data.get("ok"):
            return True, data.get("mensaje", "Abogado reactivado")
        return False, data.get("error") or LAST_ERROR or "No se pudo reactivar al abogado"
    except Exception as e:
        logger.error(f"ERROR reactivar_abogado: {e}")
        return False, str(e)


def aprobar_abogado(uid, admin_id_token=None):
    """Requiere el id_token del admin logueado: la Edge Function admin-actions
    verifica contra Firebase que sea realmente un usuario con rol='admin'
    antes de aprobar. Ver supabase/supabase/functions/admin-actions."""
    try:
        data = _invocar_admin_action("aprobar_abogado", {"uid": uid}, admin_id_token) or {}
        if data.get("ok"):
            return True, data.get("mensaje", "Abogado aprobado")
        return False, data.get("error") or LAST_ERROR or "No se pudo aprobar al abogado"
    except Exception as e:
...
def listar_retiros_pendientes(admin_id_token=None):
    """Accion de administrador (incluye CBU/alias de los abogados).
    Requiere admin_id_token, ver admin-actions."""
    try:
        data = _invocar_admin_action("listar_retiros_pendientes", {}, admin_id_token) or {}
        if not data.get("ok"):
            logger.error(f"ERROR listar_retiros_pendientes: {data.get('error') or LAST_ERROR}")
            return []
        return data.get("retiros", [])
    except Exception as e:
        logger.error(f"ERROR listar_retiros_pendientes: {e}")
        return []


def procesar_retiro(retiro_id, admin_id_token=None):
    """Accion de administrador. Requiere admin_id_token: admin-actions
    verifica rol='admin' contra Firebase antes de marcar el retiro pagado."""
    try:
        data = _invocar_admin_action("procesar_retiro", {"retiro_id": retiro_id}, admin_id_token) or {}
        if not data.get("ok"):
            return False, data.get("error") or LAST_ERROR or "No se pudo procesar el retiro"

        abogado_uid = data.get("abogado_uid")
        if abogado_uid:
            notificar_retiro_pagado(abogado_uid, data.get("monto_neto", 0))

        return True, data.get("mensaje", "Retiro pagado")
...
def actualizar_configuracion(clave, valor, admin_id_token=None):
    """Accion de administrador. Requiere admin_id_token, ver admin-actions."""
    try:
        data = _invocar_admin_action(
            "actualizar_configuracion", {"clave": clave, "valor": str(valor)}, admin_id_token
        ) or {}
        logger.info(f"actualizar_configuracion: {clave}={valor}, res={data}")
        return bool(data.get("ok"))
    except Exception as e:
